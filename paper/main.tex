...
\begin{abstract}
  ...
  DES achieves an area under the receiver operating characteristic curve of
    		\textbf{0.9436} (bootstrap 95\% CI: [0.933, 0.953]) as a binary
  hallucination classifier;
  within-family model pairwise AUROC exceeds cross-family pairwise AUROC by
  $\Delta$AUROC\textsubscript{pair} $\approx$ \textbf{0.128};
...
\textbf{0.9436} (bootstrap 95\% CI [0.933, 0.953]) as a binary
hallucination classifier (ECE = \textbf{0.0274}),
with within-family pairwise AUROC averaging $\approx 0.128$ points
