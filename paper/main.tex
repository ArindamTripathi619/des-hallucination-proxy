...
\begin{abstract}
  ...
  within-family model pairs outperform cross-family pairs by a margin of
  $\Delta$AUROC = \textbf{0.128};
...
pairwise disagreement across six large language models from five
...
%% LiteLLM rotator, 11 keys, total ~12,300 calls, CPU inference, Python 3.11
%% Consumer hardware, no GPU required — highlight zero-infrastructure framing
Total API calls: approximately 12,300 (6 models $\times$ 2,017 questions,
...
\input{../outputs/tables/table1_model_accuracy.tex}
}
...
\input{../outputs/tables/table2_calibration_metrics.tex}
...
\input{../outputs/tables/table3_classification_performance.tex}
\begin{figure}[!t]
  \centering
...
\input{../outputs/tables/table4_architecture_gap.tex}
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{figures/pdf/fig4_architecture_gap.pdf}
  \caption{AUROC by model pair type. Cross-family pairs consistently
    outperform within-family pairs, with the full 7-model DES achieving
    the highest signal.}
  \label{fig:arch_gap}
\end{figure}
...
\input{../outputs/tables/table5_qwen_ablation.tex}
...
On \emph{N}=2,017 factual questions, DES achieves AUROC of
	extbf{0.9472} as a binary hallucination classifier (ECE = 0.0587),
with within-family pairs outperforming cross-family pairs by
$\Delta$AUROC = \textbf{0.128}.
...
% ============================================================
% \begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in]{figures/author_photo.jpg}}]{Author One}
